        ...
        <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 mb-3 sm:mb-4 scrollbar-hide">
          {healthData.map((topic) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`px-3 sm:px-4 py-1.5 sm:py-2 rounded-full font-semibold text-xs sm:text-sm whitespace-nowrap transition-all duration-300 ${
        ...
        <div className="flex lg:flex-col space-x-3 lg:space-x-0 lg:space-y-3 xl:space-y-4 px-4 overflow-x-auto lg:overflow-visible w-full lg:w-auto">
          {healthData.map((topic) => (
            <motion.button
              ...
              whileTap={{ scale: 0.97 }}
              className={`px-4 py-2 rounded-lg font-semibold text-base sm:text-lg md:text-xl whitespace-nowrap lg:whitespace-normal transition-all duration-300 ${
                selectedTopic.id === topic.id
      ...
      <div className="w-full lg:w-[40%] bg-[#147140] px-4 sm:px-5 md:px-6 lg:px-8 py-6 sm:py-8 lg:py-4 flex items-center justify-center">
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className="w-full"
          >
            <div className="grid grid-cols-2 gap-x-4 sm:gap-x-5 md:gap-x-6 gap-y-4 sm:gap-y-6 md:gap-y-8 text-white text-xs sm:text-sm md:text-base">
              {selectedTopic.diseases.flat().map((disease, idx) => (
                <motion.div
                  key={idx}
                  initial={{ opacity: 0, y: 10 }}
