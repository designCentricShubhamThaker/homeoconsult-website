...
const CACHE_KEY = 'packages_cache';
const CACHE_TIMESTAMP_KEY = 'packages_cache_timestamp';
...
export default function TreatmentSelector() {
  ...
  const connectWebSocket = () => {
    try {
      ...
      ws.onopen = () => {
        console.log('✅ WebSocket connected - Packages');
      };

      ws.onmessage = (event) => {
        const message = JSON.parse(event.data);
        console.log('📨 WebSocket message - Packages:', message);
      ...
      ws.onerror = (error) => {
        console.error('❌ WebSocket error - Packages:', error);
      };

      ws.onclose = () => {
        console.log('🔌 WebSocket disconnected - Packages, reconnecting...');
        setTimeout(connectWebSocket, 3000);
      };

      wsRef.current = ws;
    } catch (error) {
      console.error('❌ Failed to create WebSocket - Packages:', error);
    }
  };
            ...
            <div className="absolute bottom-3 sm:bottom-4 md:bottom-6 lg:bottom-8 left-1/2 transform -translate-x-1/2 px-3 sm:px-4 w-full flex justify-center z-10">
              <>
                <button
                  ...
                  className="bg-[#147140] rounded-full font-normal hover:bg-emerald-700 text-white py-2 sm:py-2.5 md:py-3 px-4 sm:px-5 md:px-6 transition-all duration-300 shadow-md hover:shadow-lg text-xs sm:text-sm md:text-base"
                >
                  Book an Appointment
                </button>

                {openModal && (
                  <AppointmentModal onClose={() => setOpenModal(false)} />
                )}
              </>
